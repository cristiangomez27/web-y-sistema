...
if($_SERVER['REQUEST_METHOD']==='POST'){
  try{
    if(($_POST['action']??'')==='delete'){ $id=(int)($_POST['delete_id']??0); $data['categorias']=array_values(array_filter($data['categorias'],fn($c)=>(int)$c['id']!==$id)); cw_save($data); $ok='Categoría eliminada.'; }
    else {
      ...
      if($id>0){ foreach($data['categorias'] as &$c){ if((int)$c['id']!==$id) continue; $oldSlug=(string)$c['slug']; $newSlug=cw_slug($name); $c['nombre']=$name; $c['slug']=$newSlug; $c['url']='/'.$newSlug; if($img!=='')$c['imagen']=$img; $c['activa']=isset($_POST['activa'])?1:0; $c['menu']=isset($_POST['menu'])?1:0; $c['footer']=isset($_POST['footer'])?1:0; foreach($data['productos'] as &$p){ if((int)($p['categoria_id']??0)===$id) $p['categoria_slug']=$newSlug; } break; } unset($c,$p); cw_save($data); $ok='Categoría actualizada.'; }
