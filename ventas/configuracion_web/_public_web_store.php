...
function cw_drive_mode(): string { return (string)(cw_load()['drive']['mode'] ?? cw_load()['config']['drive_mode'] ?? 'local'); }
function cw_drive_configured(): bool { $d=cw_load(); $enabled=(bool)($d['drive']['enabled'] ?? $d['config']['drive_enabled'] ?? false); $folder=trim((string)($d['drive']['folder_id'] ?? $d['config']['drive_folder_id'] ?? '')); return $enabled && $folder!==''; }
