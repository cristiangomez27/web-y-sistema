...
function cw_upload_public_image(array $file, string $folder): string { if (($file['error'] ?? UPLOAD_ERR_NO_FILE)===UPLOAD_ERR_NO_FILE) return ''; if(($file['error']??UPLOAD_ERR_OK)!==UPLOAD_ERR_OK) throw new RuntimeException('Error al subir imagen.'); if((int)($file['size']??0)>5*1024*1024) throw new RuntimeException('Imagen supera 5MB.'); $tmp=(string)($file['tmp_name']??''); if(!str_starts_with((string)(mime_content_type($tmp)?:''),'image/')) throw new RuntimeException('Solo imágenes.'); $ext=strtolower(pathinfo((string)($file['name']??''),PATHINFO_EXTENSION)); if(!in_array($ext,['jpg','jpeg','png','webp','gif'],true)) $ext='webp'; $folder=cw_slug($folder); $disk=realpath(__DIR__.'/../..').'/uploads/web/'.$folder; if(!is_dir($disk)) mkdir($disk,0775,true); $name=$folder.'_'.time().'_'.bin2hex(random_bytes(3)).'.'.$ext; if(!move_uploaded_file($tmp,$disk.'/'.$name)) throw new RuntimeException('No se pudo guardar imagen.'); return '/ventas/uploads/web/'.$folder.'/'.$name; }
